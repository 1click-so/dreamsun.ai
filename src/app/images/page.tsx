    ...
    >
      ...
      {result.pending && !result.errorMessage && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 rounded-lg bg-surface">
          ...
          <span className="animate-pulse text-[10px] font-medium text-muted">{result.model}</span>
        </div>
      )}
      {/* Skeleton placeholder until media loads */}
      {!loaded && !result.pending && !result.errorMessage && (
...
export default function GeneratePage() {
  ...
  // Resume pending image generations on page load (from DB, not localStorage)
  const resumedRef = useRef(false);
  useEffect(() => {
    if (resumedRef.current || generationsLoading) return;
    resumedRef.current = true;

    // Clear any old localStorage pending (no longer needed - DB is source of truth)
    saveStorage(STORAGE_KEYS.pending, []);

    // Find pending image generations (url=null, created in last 2 hours)
    const twoHoursAgo = Date.now() - 2 * 60 * 60 * 1000;
    const pendingGens = dbGenerations.filter(
      (g) => g.type === "image" && !g.url && new Date(g.created_at).getTime() > twoHoursAgo
    );
    if (pendingGens.length === 0) return;

    // Create slots and resume polling for each
    const slots = pendingGens.map((g) => ({
      modelName: g.model_name || g.model_id,
      modelId: g.model_id,
      slotId: `resume_${g.id}`,
    }));
    setGeneratingSlots((prev) => [...slots, ...prev]);

    pendingGens.forEach((gen, i) => {
      ...
      (async () => {
        ...
        while (true) {
          ...
          try {
            const pollRes = await fetch(`/api/generation-poll?id=${gen.id}`);
            const pollData = await pollRes.json();

            if (pollData.status === "completed" && pollData.url) {
              updateDbGeneration(gen.id, { url: pollData.url });
              invalidateCredits();
              setGeneratingSlots((prev) => prev.filter((s) => s.slotId !== slotId));
              return;
            }
            if (pollData.status === "failed") {
              updateDbGeneration(gen.id, { url: "error" });
              invalidateCredits();
              setGeneratingSlots((prev) => prev.filter((s) => s.slotId !== slotId));
              return;
            }
  ...
  }, [dbGenerations, generationsLoading, updateDbGeneration]);
    ...
    newSlots.forEach(async (slot) => {
      ...
      try {
        ...
        if (hasRefs && effectiveModel.capability === "image-to-image") {
          ...
          if (urls.length > 0) reqBody.referenceImageUrls = urls;
        }
        ...
        // Add pending generation to gallery immediately (shows spinner)
        addDbGeneration({
          id: genId,
          type: "image",
          url: null,
          ...
          settings: { modelId: effectiveModel.id, aspectRatio, resolution: imageResolution, numImages, safetyChecker } as unknown as Record<string, unknown>,
        ...
        // Poll until complete (webhook will usually complete it first, making this instant)
        const POLL_INTERVAL = 3000;
        // eslint-disable-next-line no-constant-condition
        while (true) {
          await new Promise((r) => setTimeout(r, POLL_INTERVAL));
          try {
            const pollRes = await fetch(`/api/generation-poll?id=${genId}`);
            const pollData = await pollRes.json();

            if (pollData.status === "completed" && pollData.url) {
              updateDbGeneration(genId, { url: pollData.url });
              invalidateCredits();
