...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (isRateLimitError(error) && generationId && userId) {
      console.log(`[api/videos] Rate limited - queuing generation ${generationId} for retry`);
      try {
        const supabase = await createClient();
    ...
    console.error("Animation error (raw):", error);

    const sanitized = sanitizeError(error, "Animation failed");
    return NextResponse.json(
